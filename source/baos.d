...
class BaosLL {
  ...
  public ulong processIncomingData() {
    ...
    if(tmp.length > 0)
    {
      ...
      ft12.parse(chunk);
      //writeln("processIncomingData chunk:", chunk); 
    }
  ...
  private void onFT12Frame(FT12Frame frame) {
    ...
    // TODO linklayer mode: wrap message to KNXNet/IP header
    ...
    if (isAck) {
      ...
      ubyte[] ackBuffer = FT12Helper.compose(ackFrame);
      com.write(ackBuffer);
      // TODO: to delegate? emitCemiFrame()..
  ...
  public void switch2LL() {
    ...
    writeln("Sending 0xf6 0x00 0x08 0x01 0x34 0x10 0x01 0x00 to switch to LL mode");
